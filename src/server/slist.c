...
struct slist *
mk_slist (void *(*my_alloc) (size_t), size_t capacity)
{
...
struct slist *
sl_expand (struct slist *sl, void *(*my_alloc) (size_t), size_t delta)
{
...
void
sl_free (void (*my_free) (void *), struct slist *sl)
{
  struct sl_node *ptr;
  struct sl_node *ptr2;
...
int
append (void *data, struct slist *sl)
{
...
int
push (void *data, struct slist *sl)
{
  if (unlikely (sl->length == sl->capacity))
    return 1;
  else
...
int
find (void *data, struct slist *sl)
{
  struct sl_node *ptr;
  ptr = sl->head;
  while (ptr)
...
int
mv2head (void *data, struct slist *sl)
{
...
void
traverse (void (*visit) (struct sl_node *), struct slist *sl)
{
  struct sl_node *ptr;
  ptr = sl->head;
  while (ptr != NULL)
