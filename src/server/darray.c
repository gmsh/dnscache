...
static Bool da_has_children (DArray * d, TrieIndex s);

static Symbols *da_output_symbols (const DArray * d, TrieIndex s);

static TrieChar *da_get_state_key (const DArray * d, TrieIndex state);

static TrieIndex da_find_free_base (DArray * d, const Symbols * symbols);

static Bool da_fit_symbols (DArray * d,
			    TrieIndex base, const Symbols * symbols);
...
static Bool da_enumerate_recursive (const DArray * d,
				    TrieIndex state,
				    DAEnumFunc enum_func, void *user_data);
...
static Bool
da_check_free_cell (DArray * d, TrieIndex s)
{
  return da_extend_pool (d, s) && da_get_check (d, s) < 0;
}

static Bool
da_has_children (DArray * d, TrieIndex s)
{
  TrieIndex base;
  TrieIndex c, max_c;
...
static Symbols *
da_output_symbols (const DArray * d, TrieIndex s)
{
  Symbols *syms;
  TrieIndex base;
  TrieIndex c, max_c;
...
static TrieChar *
da_get_state_key (const DArray * d, TrieIndex state)
{
  TrieChar *key;
...
static TrieIndex
da_find_free_base (DArray * d, const Symbols * symbols)
{
  TrieChar first_sym;
  TrieIndex s;
...
static Bool
da_fit_symbols (DArray * d, TrieIndex base, const Symbols * symbols)
{
  int i;
...
static void
da_relocate_base (DArray * d, TrieIndex s, TrieIndex new_base)
{
  TrieIndex old_base;
  Symbols *symbols;
  int i;
...
static Bool
da_extend_pool (DArray * d, TrieIndex to_index)
{
  TrieIndex new_begin;
  TrieIndex i;
  TrieIndex free_tail;
...
static void
da_alloc_cell (DArray * d, TrieIndex cell)
{
  TrieIndex prev, next;
...
static void
da_free_cell (DArray * d, TrieIndex cell)
{
  TrieIndex i, prev;
