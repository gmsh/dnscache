...
static int pre_do (uint8 * buf, uint32 * total_length, uint32 * index,
		   int nbuf);
...
int
dns_search (char *buf, int nbuf, void (*f) (char *, uint32 *))
{
...
static int
tcp_dns_search (char *buf, int nbuf, void (*f) (char *, uint32 *))
{
  int i, misscount = 0;
  ...
  count = 0;
  if (FIRST_WITHOUT_ERROR == *(uint8 *) (retbuf + HEAD_LENGTH - 1))
    {				//first return without error
      for (i = 0; i < nbuf; i++)
	{
  ...
  bzero(retbuf, nbuf * 2 * sizeof (uint32) + HEAD_LENGTH * sizeof (uint8));
  //second return;
  count = 0;
  i = 0;
  while ((i = read (sockfd, retbuf + count, HEAD_LENGTH * sizeof (uint8)
 ...
 */
static int
udp_dns_search (char *buf, int nbuf, void (*f) (char *, uint32 *))
{
  int i, misscount = 0;
  uint32 tempip, total_length, index[nbuf];
  struct iovec iovec[2];
...
static void
build_head (uint8 * headbuf, uint32 total_length)
{
 ...
 */
static int
pre_do (uint8 * buf, uint32 * total_length, uint32 * index, int nbuf)
{
