...
int32
dt_push_tail (uint8 * tail, datrie_tail_pool * pool)
{
  uint32 index = pool->next_free;
  while ((strlen ((char *) tail) + 1	/* +1 for '\0'
...
uint8 *
dt_get_tail (int32 index, datrie_tail_pool * pool)
{
  return pool->pool + index;
}

void
dt_remove_tail (int32 index, datrie_tail_pool * pool)
{
  /* for the current implementation this methods is useless. */
}

void
free_datp (datrie_tail_pool * pool)
{
  if (NULL == pool)
    return;
  if (NULL != pool->pool)
...
long
write_datp_to_file (FILE * f, long offset, datrie_tail_pool * pool)
{
...
datrie_tail_pool *
read_datp_from_file (FILE * f, long offset)
{
  ...
  to_return->pool =
    (uint8 *) dc_alloc (to_return->next_free * sizeof (uint8));
